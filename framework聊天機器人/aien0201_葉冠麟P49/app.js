...
var bot = new builder.UniversalBot(connector, [
    ...
    function (session, results) {
        ...
        session.dialogData.intime = builder.EntityRecognizer.resolveTime([response]);
        builder.Prompts.time(session, "請問退房時間");
    },

    function (session, results) {
        var response = results.response;
        session.dialogData.outtime = builder.EntityRecognizer.resolveTime([response]);
        session.endDialog(`hi ${session.dialogData.Name}<br>您的連絡電話${session.dialogData.Tel}<br>${session.dialogData.room}<br>您有${session.dialogData.big}位大人${session.dialogData.small}位小孩<br>入住時間：${session.dialogData.intime}<br>退房時間：${session.dialogData.outtime}`)
    }
]);
