...
var bot = new builder.UniversalBot(connector, [
    ...
    function (session, results) {
        ...
        session.dialogData.drinkmuch = results.response;
    ...
    function (session, results) {
        ...
        if (session.dialogData.drinkitem < 10){
        session.endDialog(`hi ${session.dialogData.Name}<br>您的聯絡電話：${session.dialogData.Tel}<br>您的送貨地址：${session.dialogData.address}<br>您要點的飲料是${session.dialogData.drinkmuch}杯${session.dialogData.height}的${session.dialogData.drinkitem}<br>甜度是：${session.dialogData.sweet}<br>冰塊是：${session.dialogData.ice}<br>送達時間預計是：15分鐘之後`)
        }
        else{
            session.endDialog(`hi ${session.dialogData.Name}<br>您的聯絡電話：${session.dialogData.Tel}<br>您的送貨地址：${session.dialogData.address}<br>您要點的飲料是${session.dialogData.drinkmuch}杯${session.dialogData.height}的${session.dialogData.drinkitem}<br>甜度是：${session.dialogData.sweet}<br>冰塊是：${session.dialogData.ice}<br>送達時間預計是：30分鐘之後`)
        }
    }]
);